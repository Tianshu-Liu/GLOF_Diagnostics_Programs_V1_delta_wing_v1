...
minframe=1;     % starting image 
maxframe=80;    % ending image

Im1=imread('delta_wing_masked_1.tif');
...
for i=minframe:maxframe
    if i<1000
        imagefile=strcat('delta_wing_masked_',num2str(i),'.tif');
    elseif (i>=10) && (i<100)
        imagefile=strcat('delta_wing_masked_',num2str(i),'.jpg');
    elseif (i>=100) && (i<1000)
        imagefile=strcat('delta_wing_masked_',num2str(i),'.jpg');
    else
        imagefile=strcat('delta_wing_masked_',num2str(i),'.jpg');   
    ...
    if i<1000
        imagefile=strcat('delta_wing_masked_',num2str(i+step),'.tif');
    elseif (i>=10) && (i<100)
        imagefile=strcat('delta_wing_masked_',num2str(i+step),'.jpg');
    elseif (i>=100) && (i<1000)
        imagefile=strcat('delta_wing_masked_',num2str(i+step),'.jpg');
    else
        imagefile=strcat('delta_wing_masked_',num2str(i+step),'.jpg');   
...
if (index_masking == 1)
    BW=load('BW_delta.dat');
    [BW1,BW1] = pre_processing_a(BW,BW,scale_im,size_filter);

    value_background=0;
    tor_x=BW1.*tor_x+value_background.*(ones(size(BW1))-BW1);
    tor_y=BW1.*tor_y+value_background.*(ones(size(BW1))-BW1);
    tor_mag=BW1.*tor_mag+value_background.*(ones(size(BW1))-BW1);
    Im1=BW1.*Im1+value_background.*(ones(size(BW1))-BW1);
    Im2=BW1.*Im2+value_background.*(ones(size(BW1))-BW1);
...
% dlmwrite('tor_x_delta.dat', tor_x);
% dlmwrite('tor_y_delta.dat', tor_y);
% dlmwrite('sample_image_delta.dat', I1);
